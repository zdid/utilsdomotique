...
const FILENAME = path.resolve(path.join(__dirname,'..','automationsV2.yaml'));
...
class InternalLogger {
    ...
    log(level, ...args) {
        ...
        this.logger[level](`[AutomationsExtensionV2] ${data}`);
    }
    debug(...args) {
...
class AutomationsExtensionV2 {
    ...
    parseConfig(automations ) {
        ...
        try {
            ...
            this.logger.error("parseconfig error readfile", FILENAME,", error:"+error)
            this.logger.error("parseconfig error readfile", FILENAME,", error:"+error)
        }
        if(automations) {
        try {  
        ...
        automations = (!automations && this.automationsPrev)? this.automationsPrev: (automations || {});
...
module.exports = AutomationsExtensionV2;